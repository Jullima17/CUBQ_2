...
window.addEventListener('scroll', function() {
  const background = document.querySelector('#background-video_2'); // Corrigir o seletor aqui
  ...
  // Calcula a porcentagem da opacidade com base na posição da div em relação à altura da janela, considerando o deslocamento para baixo
  const opacity = (scrollPosition - backgroundOffset + windowHeight - offset) / backgroundHeight;

  // Ajusta a curva de transição usando a função de easing
  const easedOpacity = easeOutQuart(opacity);

  // Define a opacidade da div
  background.style.opacity = easedOpacity > 1 ? 1 : (easedOpacity > 0 ? easedOpacity : 0); // Garante que a opacidade não seja menor que 0 e maior que 1
});
